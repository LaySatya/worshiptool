...
function SlideCanvas({ slide, settings }) {
  const alignMap  = { left: 'flex-start', center: 'center', right: 'flex-end' }
  const textAlign = settings.align ?? 'center'
  const scaleFactor = 0.52   // canvas is ~52 % of actual slide px

  // Build the first line text with optional verse-number prefix
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {/* Verse number inline before first line */}
          {slide.verse_num ? (
            <>
              <span style={{ color: settings.verse_num_color, fontWeight: 700 }}>
                {slide.verse_num}
              </span>
              {'  ' + firstLine}
              {restLines.length > 0 ? '\n' + restLines.join('\n') : ''}
...
export default function PreviewPanel({ slides, settings, current, onNavigate, isLoading }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {isLoading ? (
        ...
        <SlideCanvas slide={slide} settings={settings} />
      ) : (
        <div
        ...
        >
          ...
          <p className="text-sm font-medium">Paste Bible text to preview</p>
          <p className="text-xs opacity-60">Supports Khmer numbered verses &amp; English references</p>
        </div>
      )}
      ...
      {/* Current slide text summary */}
      {slide && (
        <div className="rounded-xl bg-slate-50 border border-slate-100 px-4 py-3 text-xs text-slate-500 space-y-0.5">
          {slide.verse_num && (
            <p className="font-bold" style={{ color: settings.verse_num_color }}>
              Verse {slide.verse_num}
            </p>
          )}
          {slide.lines.map((l, i) => (
            <p key={i} className="truncate leading-relaxed">{l}</p>
          ))}
          {slide.ref && (
            <p className="pt-1 font-medium text-slate-400">{slide.ref}</p>
          )}
        </div>
      )}
    </div>
